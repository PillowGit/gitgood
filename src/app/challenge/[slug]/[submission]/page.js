...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const foundSubmission = fetchedSubmissions.find(sub => sub.submission_id === currentSubmissionId);
        if (!foundSubmission) {
  ...
  const submissionLanguage = language_mappings[currentSubmission.language] || currentSubmission.language?.toLowerCase() || 'plaintext';
  const otherSubmissions = allsubmissions.filter(sub => sub.submission_id !== currentSubmissionId);
        ...
        <div className="flex flex-col md:flex-row min-h-screen bg-[#222222] text-gray-200 pt-16 md:pt-4">
            ...
            <div className="w-full md:w-1/2 p-4 md:p-6 lg:p-8 overflow-y-auto border-r-0 md:border-r border-gray-700 flex flex-col">
                ...
                <div className="mb-6 flex-shrink-0">
                    <h2 className="text-lg font-semibold mb-2 text-gray-100">Submitted Code ({currentSubmission.language})</h2>
                    <div className="border border-gray-700 rounded-md overflow-hidden h-64"> {/* Fixed height for editor */}
                        <Editor
                            height="100%"
                ...
                <div className="flex-grow overflow-y-auto"> {/* Allow this section to scroll */}
                    <h2 className="text-lg font-semibold mb-2 text-gray-100 flex-shrink-0">Other Submissions</h2>
                    {otherSubmissions.length > 0 ? (
                        <ul className="space-y-2">
                            {otherSubmissions.map(sub => (
                                <li key={sub.submission_id} className="bg-[#2d2d2d] p-3 rounded-md border border-gray-700 hover:bg-gray-600 transition duration-150">
                                    <Link href={`/challenge/${questionSlug}/${sub.submission_id}`} className="block">
                                        <div className="flex justify-between items-center text-sm">
                                            <span className={`font-mono text-xs px-2 py-0.5 rounded ${sub.passed ? 'bg-green-700 text-green-100' : 'bg-red-700 text-red-100'}`}>
                                                {sub.passed ? 'Passed' : 'Failed'}
                                            </span>
                                            <span className="text-gray-400">{formatTimestamp(sub.date_created)}</span>
                                        </div>
                                        <div className="text-xs text-gray-300 mt-1">Language: {sub.language}</div>
                                    </Link>
                                </li>
                            ))}
                        </ul>
                    ) : (
                        <p className="text-gray-400 text-sm">No other submissions found for this question.</p>
                    )}
                </div>
            </div> {/* End Left Pane */}
            ...
            <div className="w-full md:w-1/2 p-4 md:p-6 lg:p-8 flex flex-col">
                ...
                {currentSubmission.passed ? (
                    ...
                    <div className="flex-grow bg-[#1e1e1e] border border-gray-700 rounded-md p-4 overflow-y-auto">
                        <h3 className="text-lg font-semibold text-red-400 mb-2">Execution Output / Error</h3>
